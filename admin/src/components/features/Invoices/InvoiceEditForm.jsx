...
const InvoiceEditForm = ({ invoice, hasStockPermission, onCancel, onSaved, variant = 'embedded' }) => {
    ...
    const bootstrap = async () => {
      ...
      try {
        ...
        const mappedProducts = rows.map((item) => {
          ...
          const unit = Array.isArray(bd.units) ? bd.units.find((u) => u.id === item.unit_id) : null
          ...
          return {
            ...
            product_name: product?.name || item.product_name || item.name || `Product #${item.product_id}`,
            product_code: product?.sku || product?.code || item.product_code || '',
            quantity: qty,
            item_count: qty,
        ...
        const invoicePackages = Array.isArray(pkgData) ? pkgData : pkgData ? [pkgData] : []
        const mappedPackages = invoicePackages.map((p) => ({
          ...
          gst: 0,
          discount: 0,
          total_price: parseFloat(p.package_price || 0) * parseFloat(p.quantity || 1),
          stock_quantity: 0,
          stock_id: null,
        }))
  ...
  const handleUpdateStore = async (storeData) => {
    ...
    try {
      ...
      if (response.data?.status === true || response.data?.data) {
        ...
        setStores((prev) =>
          prev.map((s) => (s.id === editingStore.id ? { ...s, ...updated, ...storeData } : s))
        )
  ...
  const handleCreateStore = async (responsePayload) => {
    ...
    try {
      const info = responsePayload?.data?.data || responsePayload?.data || responsePayload
  ...
  const handleAddItem = async (product) => {
    const idx = lineItems.findIndex((item) => !item.is_package && item.product_id === product.id)
    if (idx !== -1) {
      const item = lineItems[idx]
      const newQty = parseFloat(item.quantity) + 1
      if (hasStockPermission && item.stock_quantity < Infinity && newQty > item.stock_quantity) {
        toast.error(`Max stock for this line: ${item.stock_quantity}`)
        return
      }
      setLineItems((prev) => {
        const next = [...prev]
        next[idx] = {
          ...
          quantity: newQty,
          item_count: newQty,
          total_price: calculateItemTotal(item.price, newQty, item.gst, item.discount),
        }
        return next
      })
    ...
    const newItem = {
      ...
      gst,
      discount,
      total_price: calculateItemTotal(sellingPrice, qty, gst, discount),
      status: 'completed',
  ...
  const handleUpdateItem = (index, field, value) => {
    ...
    setLineItems((prev) => {
      ...
      row.total_price = calculateItemTotal(row.price, row.quantity, row.gst, row.discount)
      next[index] = row
      return next
    })
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (hasStockPermission) {
      ...
      if (missing.length) {
        toast.error('Each product line needs stock when stock management is enabled')
        return
      }
    }
    ...
    const pkgSnapNow = JSON.stringify(
      lineItems
        ...
        .map((m) => ({ id: m.package_row_id, q: m.quantity, name: m.product_name }))
    )
      ...
      if (res.data?.status === true) {
        ...
        try {
          ...
          ch.postMessage({
            ...
            data: { customer_id: Number(formData.customer_id), timestamp: Date.now() },
          })
          ch.close()
        } catch {
          /* ignore */
  ...
  useEffect(() => {
    const onDocClick = (ev) => {
      if (!ev.target.closest('.product-dropdown')) setShowProductList(false)
      if (!ev.target.closest('.package-dropdown')) setShowPackageDropdown(false)
    }
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Invoice #{invoice.invoice_number || invoice.id} — same layout as bill generation
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            <Button type="button" variant="outline" onClick={onCancel} icon={FiX} disabled={saving}>
              Cancel
            </Button>
            <Button
              type="submit"
        ...
        {dataFetchError && (
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 text-yellow-800 dark:text-yellow-200 flex items-center gap-2">
            ...
            <span>Unable to load all data. Check your connection and try again.</span>
          </div>
        )}
              ...
              <div className="space-y-4">
                <SearchSelect
                  ...
                  onChange={(value) => {
                    ...
                    if (c) toast.success(`Customer: ${c.name || c.customer_name}`)
                  }}
                  ...
                  onCreateNew={(searchTerm) => {
                    ...
                    if (isPhone) {
                      setNewCustomerData((p) => ({ ...p, phone: searchTerm.trim(), name: '' }))
                    } else {
                      setNewCustomerData((p) => ({ ...p, name: searchTerm.trim(), phone: '' }))
                    }
                ...
                <SearchSelect
                  ...
                  options={(stores || []).map((store) => ({
                    ...
                    description:
                      store.mobile || store.phone || store.email
                        ...
                        : null,
                    subtext:
                      store.address && store.city ? `📍 ${store.address}, ${store.city}` : null,
                    rightContent: (
                      <button
                        type="button"
                        onClick={(e) => {
                  ...
                  onCreateNew={(searchTerm) => {
                    if (stores.length > 0) {
                      toast.error('You already have a store. Only one store is allowed.')
                      return
                    }
                    const isPhone = /^\d[\d\s-]*$/.test(searchTerm.trim())
                    if (isPhone) {
                      setNewStoreData((p) => ({ ...p, mobile: searchTerm.trim(), name: '' }))
                    } else {
                      setNewStoreData((p) => ({ ...p, name: searchTerm.trim(), mobile: '' }))
                    }
                    ...
                    {filteredPackages.length > 0 ? (
                      filteredPackages.map((pkg) => (
                        <div
                          ...
                          onKeyDown={(ev) => ev.key === 'Enter' && handlePackageSelect(pkg)}
                        ...
                        >
                          <div className="flex justify-between gap-2">
                            <div>
                              ...
                              <div className="text-sm text-gray-500">Size: {pkg.package_size || '—'}</div>
                            </div>
                      ...
                      <div className="px-4 py-3 text-center text-gray-500">No packages found</div>
                    )}
                  </div>
                )}
                  ...
                  <div className="p-4 bg-white dark:bg-gray-600 rounded-lg border border-gray-200 dark:border-gray-500 mt-2">
                    <div className="flex justify-between gap-2 mb-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">Size: {selectedPackage.package_size || '—'}</p>
                      </div>
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <Input
                        ...
                        onChange={(e) => setPackageQuantity(parseInt(e.target.value, 10) || 1)}
                        className="w-20"
                      />
                      <Button
                        type="button"
                    ...
                    {filteredProducts.length > 0 ? (
                      filteredProducts.map((product) => (
                        <div
                          ...
                          onKeyDown={(ev) => ev.key === 'Enter' && handleAddItem(product)}
                        >
                          <div className="flex justify-between gap-2">
                            <div>
                              <div className="font-medium text-gray-900 dark:text-white">
                              ...
                              <div className="text-sm text-gray-500 space-y-1 mt-1">
                                ...
                                {product.brand?.name && <div>Brand: {product.brand.name}</div>}
                              </div>
                            </div>
            ...
            {productLinesOnly.length > 0 && (
              <div className="mt-4">
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Product lines ({productLinesOnly.length})
                </p>
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {productLinesOnly.map((item) => {
                    const globalIdx = lineItems.indexOf(item)
                    return (
                      <div
                        key={`${item.product_id}-${globalIdx}`}
                        className="flex items-center justify-between p-2 bg-white dark:bg-gray-600 rounded-lg border border-gray-200 dark:border-gray-500 text-sm"
                      >
              ...
              <table className="w-full min-w-[800px]">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-600">
                    <th className="text-left text-xs font-medium text-gray-500 pb-3">Product / Package</th>
                    <th className="text-center text-xs font-medium text-gray-500 pb-3 w-[100px]">Qty</th>
                    <th className="text-center text-xs font-medium text-gray-500 pb-3 w-[100px]">Price</th>
                    <th className="text-center text-xs font-medium text-gray-500 pb-3 w-[100px]">GST %</th>
                    ...
                    >
                      <td className="py-3">
                        <div className="space-y-1">
                          ...
                          {!item.is_package && item.stock_quantity !== undefined && item.stock_quantity < Infinity && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="p-4 bg-white dark:bg-gray-600 rounded-lg border border-gray-200 dark:border-gray-500 space-y-2 text-sm">
                <p className="text-xs text-gray-500 dark:text-gray-400 pb-2 border-b border-gray-100 dark:border-gray-500/60">
                  Saved invoice when you opened edit — Total ₹
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600 dark:text-gray-300">Product lines (after discount + GST)</span>
                ...
                <div className="flex justify-between font-bold text-base pt-2 border-t border-gray-200 dark:border-gray-500">
