...
const Sidebar = () => {
  ...
  const menuItems = sidebarPermissions.map(permission => {
    ...
    const icon = iconMap[permission.slug] || FiSettings;
    const badge = permission.slug === 'plans' && planExpireReminder ? '⚠️' : null;
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Item name with smooth animation */}
                  <AnimatePresence mode="wait">
                    {sidebarOpen && (
                      <motion.span
                  ...
                  {/* Tooltip for collapsed state with smooth animation */}
                  <AnimatePresence>
                    {!sidebarOpen && !isMobile && hoveredItem === item.path && (
                      <motion.div
                        ...
                        animate="animate"
                        exit="exit"
                        className="absolute left-full ml-2 px-3 py-2 bg-gray-900 dark:bg-gray-700 text-white text-sm rounded-lg whitespace-nowrap shadow-lg z-50 pointer-events-none"
                      >
                        {item.name}
                        {item.badge && (
