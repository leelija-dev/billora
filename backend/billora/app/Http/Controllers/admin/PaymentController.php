...
// ]);
        $response = Http::withHeaders([
            ...
            "order_id" => $orderId,
            "order_amount" => round($totalPriceWithGst, 2), //$request->amount,
            "order_currency" => "INR",
            "customer_details" => [
    ...
    public function renewPlan(Request $request)
    {
        ...
        $gstAmt = (($plan->price - $disAmt) * $plan->gst) / 100;
        $amt = (($plan->price - $disAmt) + $gstAmt); 
        // Log::info('Renew Plan Amount: ' . $amt);
        // Log::info('Renew Plan GST Amount: ' . $gstAmt);
        // Log::info('Renew Plan Discount Amount: ' . $disAmt);
        // Log::info('Renew Plan Order Amount: ' . $data['amount']);
        $response = Http::withHeaders([
                'x-client-id' => config('cashfree.app_id'),
                'x-client-secret' => config('cashfree.secret_key'),
                'x-api-version' => '2022-09-01',
