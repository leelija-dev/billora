  ...
  return (
    <nav className="sticky top-0 bg-white shadow-md z-[1000] h-20 flex items-center px-5 lg:px-10">
      <div className="max-w-[1400px] w-full mx-auto flex justify-between items-center h-full">
        ...
        {/* Logo */}
        <a href="/" className="flex items-center gap-2.5 transition-transform hover:scale-105" onClick={handleLogoClick}>
          <span className="bg-blue-600 text-white px-2.5 py-1 rounded text-2xl lg:text-3xl font-bold leading-none">V</span>
          <span className="text-slate-800 text-2xl lg:text-3xl font-bold leading-none">Vyapar</span>
        </a>

        {/* Mobile Toggle Button */}
        <button 
          className="flex lg:hidden flex-col gap-1.5 p-2 z-[1001]"
          onClick={() => setIsMenuOpen(!isMenuOpen)}
          aria-label="Toggle menu"
        >
          <span className={`w-6 h-0.5 bg-slate-800 transition-all ${isMenuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
          <span className={`w-6 h-0.5 bg-slate-800 transition-all ${isMenuOpen ? 'opacity-0' : ''}`}></span>
          <span className={`w-6 h-0.5 bg-slate-800 transition-all ${isMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
        </button>

        {/* Navigation Menu */}
        <div className={`
          fixed lg:static top-20 left-0 right-0 bg-white lg:bg-transparent
          ...
          ${isMenuOpen ? 'max-h-[600px] opacity-100 visible' : 'max-h-0 opacity-0 invisible lg:max-h-none lg:opacity-100 lg:visible'}
        `}>
          <ul className="flex flex-col lg:flex-row list-none gap-0 lg:gap-5 m-0 p-0 items-start lg:items-center">
            {['try-mobile-app', 'solution', 'about-us', 'desktop', 'careers', 'partner'].map((item) => (
              <li key={item} className="w-full lg:w-auto border-b lg:border-none border-slate-100">
                <a 
                  href={`#${item}`} 
                  className="block py-4 lg:py-0 text-slate-500 font-medium text-sm transition-colors hover:text-blue-600 lg:border-b-2 lg:border-transparent lg:hover:border-blue-600 capitalize whitespace-nowrap"
            ...
            <li className="w-full lg:w-auto border-b lg:border-none border-slate-100">
              <button 
                ...
                className="block py-4 lg:py-0 text-slate-500 font-medium text-sm hover:text-blue-600"
              >
                Pricing
              </button>
            </li>
