...
import { Folder, ArrowLeft, Clock, Eye, Hash, Grid, List, BookOpen } from 'lucide-react';
...
export default function CategoryClient({ initialData, categorySlug, categoryName }) {
  ...
  const fetchBlogsByCategory = useCallback(async (page = 1, reset = false) => {
    try {
      if (reset) {
        setLoading(true);
      } else {
  ...
  useEffect(() => {
    if (!initialData && !initialFetchDone.current) {
      initialFetchDone.current = true;
      fetchBlogsByCategory(1, true);
    }
  }, [initialData, fetchBlogsByCategory]);

  const handleLoadMore = () => {
    if (!loadingMore && hasMore) {
      fetchBlogsByCategory(currentPage + 1, false);
    }
  };
        ...
        <div className="container  mx-auto">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white rounded-3xl border border-slate-200/50 shadow-lg">
              ...
              <button
                onClick={() => fetchBlogsByCategory(1, true)}
              ...
              {hasMore && (
                <div className="text-center mt-12">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-medium shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                  >
                    {loadingMore ? (
                      <>
