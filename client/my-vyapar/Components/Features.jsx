...
const Features = () => {
  ...
  return (
    <section className="py-16 md:py-24 lg:py-[100px] bg-white font-sans overflow-hidden relative">
      ...
      {/* Animated Circle Elements - Responsive Sizes */}
      <div className="absolute top-10 sm:top-20 -left-24 sm:-left-20 w-48 sm:w-72 h-48 sm:h-72 bg-gradient-to-br from-blue-400 to-blue-200 rounded-full circle-float-1 blur-2xl"></div>
      <div className="absolute top-1/4 -right-32 sm:right-20 w-56 sm:w-80 h-56 sm:h-80 bg-gradient-to-br from-purple-400 to-purple-200 rounded-full circle-float-2 blur-2xl"></div>
      <div className="absolute -bottom-20 sm:bottom-40 left-1/3 sm:left-1/4 w-64 sm:w-96 h-64 sm:h-96 bg-gradient-to-br from-indigo-400 to-indigo-200 rounded-full circle-float-3 blur-2xl"></div>
      <div className="absolute -bottom-32 sm:bottom-10 right-1/3 w-48 sm:w-64 h-48 sm:h-64 bg-gradient-to-br from-cyan-400 to-cyan-200 rounded-full circle-float-4 blur-2xl"></div>

      {/* Content */}
      <div className="relative z-10">
        {/* Header Section */}
        <div className="text-center mb-12 sm:mb-16 md:mb-20 lg:mb-[60px] max-w-full sm:max-w-[700px] md:max-w-[800px] mx-auto px-4 sm:px-5">
        ...
        <div className="max-w-full sm:max-w-[900px] md:max-w-[1200px] mx-auto px-4 sm:px-5">
          {features.map((feature, index) => {
            ...
            return (
              <div
                ...
                data-index={index}
                className={`flex flex-col lg:flex-row items-center gap-6 sm:gap-10 md:gap-16 lg:gap-[60px] mb-12 sm:mb-16 md:mb-20 lg:mb-[100px] transition-all duration-1000 ${
                  feature.reverse ? 'lg:flex-row-reverse' : ''
                }`}
              >
                {/* Image with animation */}
                <div 
                  className={`w-full sm:flex-1 flex justify-center items-center transition-all duration-1000 ${
                    visibleItems[index]
                ...
                >
                  <div className="w-full max-w-xs sm:max-w-sm md:max-w-md overflow-hidden rounded-2xl shadow-lg hover:shadow-2xl transition-shadow duration-300">
                    <Image
                      src={feature.image}
                      alt={feature.title}
                      width={500}
                ...
                <div 
                  className={`w-full sm:flex-1 transition-all duration-1000 delay-200 ${
                    visibleItems[index]
                      ? 'opacity-100 translate-x-0'
                      : isEven 
                        ? 'opacity-0 translate-x-20' 
                ...
                >
                  <h3 className="text-xl sm:text-2xl md:text-[28px] text-black mb-3 sm:mb-4 font-semibold leading-snug transition-colors duration-300 hover:text-[#2f5fa5]">
                    {feature.title}
                  </h3>
                  
                  <p className="text-sm sm:text-base text-[#555] leading-relaxed sm:leading-[1.8] m-0">
                    {feature.description}
                  </p>
                  
                  {/* Extra paragraph that appears when expanded */}
                  {expandedItems[index] && (
                    <p className="text-sm sm:text-base text-[#555] leading-relaxed sm:leading-[1.8] mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-[#2f5fa5]/20 animate-in slide-in-from-top duration-500">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 mt-3 sm:mt-4 text-[#2f5fa5] font-medium text-sm sm:text-base group hover:cursor-pointer bg-transparent border-none p-0 transition-all duration-300 hover:gap-3"
                  >
