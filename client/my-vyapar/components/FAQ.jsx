        ...
        <div className="flex-1 w-full ">
          {faqs.map((item, index) => (
            ...
            >
              ...
              >
                <p className={`text-sm sm:text-base md:text-lg font-semibold text-slate-900 leading-relaxed transition-colors ${activeIndex === index ? "text-[#1e88e5]" : "group-hover:text-[#1e88e5]"
                  }`}>
                  {item.question}
                </p>
                <span
                  className={`text-xl sm:text-2xl md:text-3xl text-[#1e88e5] transition-transform duration-300 flex-shrink-0 font-light ${activeIndex === index ? "rotate-180" : ""
                    }`}
                >
                  {activeIndex === index ? "−" : "+"}
                </span>
